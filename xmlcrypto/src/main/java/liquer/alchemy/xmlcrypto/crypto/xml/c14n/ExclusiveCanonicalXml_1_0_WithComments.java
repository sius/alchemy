...
public class ExclusiveCanonicalXml_1_0_WithComments implements CanonicalXml {
    ...
    public Node apply(Node node, CanonicalOptions options) throws CanonicalizationException {
        try {
            final CanonicalOptions finalOptions = options == null ? new CanonicalOptions() : options;

            List<String> inclusiveNamespacesPrefixList = finalOptions.getInclusiveNamespacesPrefixList();
            List<PrefixNamespaceTuple> ancestorNamespaces = finalOptions.getAncestorNamespaces();
            ...
            StringBuilder res = buildRecursively(
                node,
                new ArrayList<>(),
                finalOptions.getDefaultNamespaceURI(),
                finalOptions.getDefaultNsForPrefix(),
                finalOptions.getInclusiveNamespacesPrefixList());
    ...
    private NamespacesTuple buildNamespaces(Node node, List<PrefixNamespaceTuple> prefixesInScope, String defaultNamespaceURI, Map<String, String> defaultNsForPrefix, List<String> inclusiveNamespacesPrefixList) {
        ...
        for (int i = 0; i < attributes.getLength(); ++i) {
            ...
            if (StringSupport.notNullOrEmpty(attr.getPrefix())) {
