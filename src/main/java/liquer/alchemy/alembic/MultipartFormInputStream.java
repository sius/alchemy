 ...
 * @version $Id: MultipartFormInputStream.java 761957 2009-04-04 15:14:16Z jdonnerstag $
 */
...
public final class MultipartFormInputStream {
	 ...
	 * bytes.).
	 */
	public final static int HEADER_PART_SIZE_MAX = 10240;

	/**
	 * The default length of the buffer used for processing a request.
	 */
	public final static int DEFAULT_BUFSIZE = 1024 << 6; // 64 KB //4096;

	/**
	 * A byte sequence that marks the end of <code>header-part</code> (<code>CRLFCRLF</code>).
	 */
	final static byte[] HEADER_SEPARATOR = { CR, LF, CR, LF };

	/**
	 * The HEADER_SEPARATOR as String value
	 */
	public final static String HEADER_DELIMITER = StringSupport.decodeBytesToString(HEADER_SEPARATOR, Charset.defaultCharset());

	/**
	 * A byte sequence that that follows a delimiter that will be followed by an encapsulation (
	 * <code>CRLF</code>).
	 */
	final static byte[] FIELD_SEPARATOR = { CR, LF };

	/**
	 * The FIELD_SEPARATOR as String value
	 */
	public final static String FIELD_DELIMITER = StringSupport.decodeBytesToString(HEADER_SEPARATOR, Charset.defaultCharset());

	/**
	 * A byte sequence that that follows a delimiter of the last encapsulation in the stream (
	 * <code>--</code>).
	 */
	final static byte[] STREAM_TERMINATOR = { DASH, DASH };
	 ...
	 * @exception MalformedStreamException if the stream ends unexpectedly or fails to follow required syntax.
	 */
	 ...
	 *
	 * @param maxSize The maximum amount to read before giving up
	...
	public String readHeaders(final int maxSize) throws MalformedStreamException {
		...
		if (headerEncoding == null) {
			headers = StringSupport.decodeBytesToString(baos.toByteArray(), Charset.defaultCharset());
		} else {
			headers = StringSupport.decodeBytesToString(baos.toByteArray(), headerEncoding);
		}
	...
	/**
	 * <p>
	 * Reads <code>body-data</code> from the current <code>encapsulation</code> and return its
	 * contents as String.
	 * <p>
	 *
	 ...
	 */
	public String readBodyDataToString(String charsetName) throws IOException {
		ByteArrayOutputStream ret = new ByteArrayOutputStream();
		readBodyData(ret);
		return StringSupport.decodeBytesToString(ret.toByteArray(), charsetName);
	}
	/**
	 * <p>
