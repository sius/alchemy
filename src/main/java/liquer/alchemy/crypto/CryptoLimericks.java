...
public final class CryptoLimericks {
    ...
    /**
     * if the PBKDF2WithHmacSHA1 cryptographic provider is missing (WAS6.1)
     * @param passPhrase passPhrase
     * @param salt salt
     * @param iterations iterations
     * @return the crypto digest
     */
    public static byte[] cryptoDigestCPBKDF2(char[] passPhrase,  byte[] salt, int iterations, int hashBytes)  {
        return CPbkdf2.derive(passPhrase, salt, iterations, hashBytes * 8);
    }
}
